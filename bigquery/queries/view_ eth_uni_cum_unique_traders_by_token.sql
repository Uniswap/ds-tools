...
-- this query computes cumulative unique number of traders for each token on Uniswap, V2 + V3
...
with union_23 as (
    ...
    SELECT  -- this is for V2
            ...
            DATE(v2.block_timestamp) AS trade_time,
            v2.transaction_hash,
            v2.to as swap_to_address,
            v2_fac.token0,
            v2_fac.token1,
            '2' as version
    FROM `mimetic-design-338620.uniswap.Swap_V2`  v2
    ...
    SELECT  -- this is for V3
        address as pool_address,
        DATE(v3.block_timestamp) AS trade_time,
        v3.transaction_hash,
        v3.recipient as swap_to_address,
        v3_fac.token0,
        v3_fac.token1,
        '3' as version
    FROM `mimetic-design-338620.uniswap.swap`  v3
    ...
    WHERE DATE(v3.block_timestamp) > from_date
     ...
     token_trade_history as (
         SELECT
             union_23.pool_address,
             union_23.trade_time,
             union_23.transaction_hash,
             union_23.swap_to_address,
     ...
     unique_trader_count_by_date_by_token as (
         SELECT
             token,
             first_trade_time as as_of_date,
             ROW_NUMBER() OVER(PARTITION BY token
                 ORDER BY first_trade_time ASC) AS count_user
         FROM first_trade_by_user
     ),

     cumulative_unique_trader_count_by_date_by_token_with_missing_dates as (
         SELECT
             token,
             as_of_date,
             max(count_user) as cumulative_unique_user
         FROM unique_trader_count_by_date_by_token
     ...
     token_date_range as (
         ...
         FROM cumulative_unique_trader_count_by_date_by_token_with_missing_dates
         GROUP BY token
     )
        ,
         ...
         (
             SELECT
                 ...
                 ROW_NUMBER() OVER
                     ...
                     ) AS RowNum -- this logic is to get the most recent cumulative trader count
             FROM token_in_range_dates t1
                      INNER JOIN cumulative_unique_trader_count_by_date_by_token_with_missing_dates t2
                                 ON t1.token = t2.token
                                     AND t2.as_of_date <= t1.calendar_date
         )

select * from trader_count_date_up_to
WHERE RowNum = 1
